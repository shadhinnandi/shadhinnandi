...
const setActiveLink = (activeId)=>{
    navLinks.forEach((link)=>{
        if (link.getAttribute("href") === `#${activeId}`) link.classList.add("is-active");
...
if (navLinks.length) {
    ...
    navLinks.forEach((link)=>{
        link.addEventListener("click", ()=>{
            ...
            if (hash && hash.startsWith("#")) setActiveLink(hash.substring(1));
        });
    });
    if ("IntersectionObserver" in window && anchorSections.length) {
        ...
        onScroll();
    } else // default to first nav link
    setActiveLink(navLinks[0].getAttribute("href").replace("#", ""));
    // Ensure a link is highlighted on initial load
    if (anchorSections.length) setActiveLink(anchorSections[0].id);
    else setActiveLink(navLinks[0].getAttribute("href").replace("#", ""));
}
